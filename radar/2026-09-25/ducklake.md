...
That makes the stack considerably simpler: a database we already run replaces the catalog, and transactions across tables come for free. We are preparing a pilot next to our Iceberg lakehouse.

### Considerations

- **Young format:** Engine support outside DuckDB is still limited, whereas Iceberg is supported almost everywhere. For now Iceberg remains our default.
